...
actually run:
  ...
  2. a ±3·σ rolling band is the **tolerance envelope** (a `Spread`);
  ...
  4. a second, X-linked panel plots the **residual** (signal − baseline) — derived on
     the fly with a qtviz `Expression`, no extra column needed.
...
def build(theme: qv.Theme | None = None):
    ...
    signal = qv.Curve(df, x="t", y="temp", color="#9ecbff", line_width=1.0)
