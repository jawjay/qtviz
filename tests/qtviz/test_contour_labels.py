...
def test_validation():
    ...
    qv.Contour(_FIELD, bounds=_BOUNDS, labels=True)
    qv.Contour(_FIELD, bounds=_BOUNDS, labels=".2f")
    with pytest.raises(ValidationError):
        qv.Contour(_FIELD, bounds=_BOUNDS, labels="{bad}{spec}")
...
def test_webengine_labels_are_rotated_annotations_with_mask():
    ...
    el = qv.Contour(_FIELD, bounds=_BOUNDS, levels=[0.3, 0.6], labels=True)
...
def test_backends_place_labels_identically(qtbot):
    ...
    el = qv.Contour(_FIELD, bounds=_BOUNDS, levels=[0.5], labels=True)
