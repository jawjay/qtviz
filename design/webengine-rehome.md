...
| **W5.2** (Phase 5) — **deferred** | Scale + offline — custom `qtviz://` URL scheme serves **raw binary** data buffers **and** the bundled plotly.js (no per-page JS bloat); raw typed arrays, no apache-arrow ([[D34]] revised) | <100 ms for ~100 MB; **resume guide:** `webengine-arrow-transport.md` §10 |
