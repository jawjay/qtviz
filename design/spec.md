...
class Element:
    ...
    def _fields(self) -> dict[str, Any]:
        """Returns the public attrs (non-underscore). Used by __eq__,
        __hash__, .with_(), __repr__."""
    ...
    def __hash__(self) -> int:
        return hash((type(self), tuple(sorted(self._fields().items(), key=str))))
...
```python
DataRef = Union[
    ArrayLike,           # numpy ndarray, pandas DataFrame/Series
    pa.Table,            # arrow
    DataSource,          # lazy source (Phase 5)
    Signal[Any],         # reactive (Phase 4)
]
```

Each ref implements:

- `.snapshot() -> ConcreteData` — get the current value (sync on the GUI
  thread; for `DataSource` see §2.14 on worker delegation).
- `.subscribe(cb) -> Disposable` — fire `cb` when underlying data changes
  (no-op for static refs).

The Element calls `.snapshot()` at render time. Reactive subscription
is orchestrated by `View`, not the Element itself (§2.9).
...
- Auto-route at scale (`Scatter(table, scale="auto")` picks Datashader
  if `len(table) > backend.capabilities.max_recommended_points`).
...
Throttle implementation is the same trailing-edge pattern already in
`qtviz.backends.webengine.WebBridgeView._Throttle` — promoted to
`qtviz.core.event` so all backends use it.

`View.on(event_type, cb, throttle_ms=...)` forwards to
`handle.event_bus.subscribe`. The bus lives on the handle so disposal
...
A small registry holds built-ins:

```python
qtviz.palettes.register("viridis", Palette.from_matplotlib("viridis"))
...
The user adds one palette with one call.

### 2.13 Theme

Already exists (`qtviz.core.theme.Theme`). Carries background /
foreground / grid color / `Palette` / font family + sizes. Each backend
has an `apply_theme(widget, theme)` that walks its widget tree.

Theme is applied at render time and on `set_theme` on the View. Theme
is **not** part of `Element` — Elements are theme-agnostic. Theme lives
in `RenderContext`.
...
- `DataSource.snapshot()` (Phase 5): the *public* call returns a
  `Future`; the actual I/O runs on a worker. The View handles the
  Future and re-renders when ready.
...
Deterministic; runs once per `set_root`. Errors are loud and actionable.
Lives in `qtviz.core.compose` per Q9 (resolved).

### 3.3 Auto mode

`backend="auto"` picks a backend per node based on capabilities:

```python
def auto_backend(node):
    if isinstance(node, Element):
        n = data_size(node)
        candidates = [b for b in registered if b.supports(type(node))]
        if not candidates:
            raise NoBackendFor(node)
        if n > 1_000_000:
            # prefer backend with highest max_recommended_points
            return max(candidates, key=lambda b: b.capabilities.max_recommended_points)
        # otherwise prefer by user-tunable priority
        return max(candidates, key=lambda b: -priority_index(b.name))
```

Priority order (default, per Q10 resolved): `pyqtgraph` > `matplotlib`
> `webengine`. Tunable via `qtviz.set_backend_priority([...])`.

This is a **convenience** layer — never silent magic. Whatever auto
picks is logged at INFO and visible on the handle
(`handle.backend_name`).
...
Indexed by `x`/`y` columns into a regular grid. Shape like Image but
backend-driven from a tabular data ref.
...
## 6. Data binding (Phase 1 minimum)

Phase 1 supports two `DataRef` kinds:

- **In-memory**: numpy array, pandas DataFrame, Arrow Table. `.snapshot()`
  returns the data directly (sync, GUI thread).
- **`Signal[Data]`** (Phase 4): reactive. `.snapshot()` reads the current
  value; `.subscribe(cb)` triggers re-render on changes. Cross-thread
  `Signal.set` is supported per §2.14. **`Signal` requires an initial
  value at construction** — there is no "uninitialized" state
  (Q-B resolved). `None` is allowed only when the snapshot type
  permits it explicitly (e.g., `Signal[pa.Table | None]`).

`DataSource` (lazy, query-backed) lands in Phase 5. Element doesn't
need to change — only the DataRef union grows. The threading model
already accommodates it (snapshot returns a Future from a worker).
...
None as of this revision. New questions go here as they arise during
implementation.
...
For Phase 1, the concrete files to create, in order:

1. `qtviz/core/_immutable.py` — `_freeze` / `__setattr__` / `with_` /
   `__eq__` / `__hash__` / `_fields` mixin used by Element, Options,
   Overlay, Layout, Palette.
2. `qtviz/core/data_ref.py` — DataRef union, `snapshot` protocol.
...
8. `qtviz/core/event.py` — `Event` types, `EventBus`, throttle.
9. `qtviz/core/threading.py` — `require_gui_thread`, `run_on_gui`, `Worker`.
10. `qtviz/core/backend.py` — `Backend` Protocol, `Capabilities`,
    `RendererRegistry`, `RenderHandle`, `RenderContext`.
11. `qtviz/core/view.py` — `View` widget.
12. `qtviz/core/theme.py` — exists today; minor cleanup.
